 ...
 */
export function unprotectCommand(port: number, options: GlobalOptions): Effect.Effect<string, Error> {
  return Effect.gen(function* () {
    ...
    return options.json ? renderJson({ protectedPorts: config.protectedPorts }) : `已取消保护端口 ${port}。`;
  });
}
